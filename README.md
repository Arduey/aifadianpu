...
3. **上传代码**到站点目录,如 `/www/wwwroot/afdianpu`,然后:
   ```bash
   cd /www/wwwroot/afdianpu
   cp .env.example .env   # 编辑:DB_*、SESSION_SECRET、APP_BASE_URL、SMTP_*
   ```
4. **Python 项目管理器 → 添加项目**:
   ...
   - 启动项目。**数据表会在首次启动时自动创建**(幂等;也可用 phpMyAdmin 导入 `database/schema.sql`)。
...
6. **首次使用**:打开域名 → 注册第一个账号即管理员 → 个人中心配置爱发电(消费者)账号与平台设置 → 商品管理创建「API充值」档位 → 分享店铺链接。
